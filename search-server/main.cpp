...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (auto st_w : stop_words_) {
            if (!IsValidWord(st_w)) {
                throw invalid_argument("Спецсимволы запрещены");
            }
        }
    ...
    // Непонятно зачем в условии задачи эта функия? Но раз требуют - написал.
...
int main() {
    ...
    SearchServer search_server("и в на проба"s);
